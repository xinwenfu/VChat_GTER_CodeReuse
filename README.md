...
```
* `s`: This argument specifies the socket descriptor of a socket that is to be used when making the connection. This specifies the source port.
* `name`: This argument specifies a pointer to the [sockaddr](https://learn.microsoft.com/en-us/windows/win32/winsock/sockaddr-2) structure we use to specify the destination of this connection.
* `namelen`: This argument specifies the length of the `name` argument; depending on the socket type, we may use a different `sockaddr` structure.
...
```
* `lpApplicationName`: This is the full path to the module or executable to be executed, if this is NULL the first whitespace delimitated token in `lpCommandLine` must be the executable's name.
* `lpCommandLine`: This is the command to be executed, if this is null it uses the `lpApplicationName` parameter as a argument
* `lpProcessAttributes`: A pointer to the [SECURITY_ATTRIBUTES](https://learn.microsoft.com/en-us/previous-versions/windows/desktop/legacy/aa379560(v=vs.85)) structure. This can be NULL. 
* `lpThreadAttributes`: A pointer to the [SECURITY_ATTRIBUTES](https://learn.microsoft.com/en-us/previous-versions/windows/desktop/legacy/aa379560(v=vs.85)) structure. This can be NULL. 
* `bInheritHandles`: If set to true all inheritable handles will be inherited by the child process. Otherwise they will not be. 
* `dwCreationFlags`: Used to configure Process creation attributes such as priority through the [process-creation-flags](https://learn.microsoft.com/en-us/windows/win32/procthread/process-creation-flags).
* `lpEnvironment`: This is used to control the environment block of the spawned processes. If NULL it will inherit from the parent process.
* `lpCurrentDirectory`: Full path to the directory the preocess should have as the Current Working Directory (CWD), if NULL it will share the CWD of the parent process
* `lpStartupInfo`: Pointer to [STARTUPINFOA](https://learn.microsoft.com/en-us/windows/win32/api/processthreadsapi/ns-processthreadsapi-startupinfoa) structure.
* `lpProcessInformation`: Pointer to [PROCESS_INFORMATION](https://learn.microsoft.com/en-us/windows/win32/api/processthreadsapi/ns-processthreadsapi-process_information) structure. The function will write to the structure (pointer) we provide.
  

The `_STARTUPINFOA` structure will be used to make sure our process reads from and writes to the socket we have created. Its definition is described below:
```
...
```
* `c`: Size of the structure in bytes
* `lpReserved`: Must be NULL (NULL)
* `lpDesktop`: Name of desktop or windows station for this process (NULL).
* `lpTitle`: Name to be displayed in title bar; if NULL this is the executable's file name is used (NULL).
* `dwX`: Start position x, if no GUI window is created this should be NULL. 
* `dwY`: Start position y, if no GUI window is created this should be NULL. 
* `dwXSize`: Width of window, if no GUI window is created this is ignored (NULL).
* `dwYSize`: Height of window, if no GUI window is created this is ignored (NULL).
* `dwXCountChars`: Specifies the screen buffer width, if no GUI window is created this is ignored (NULL).
* `dwYCountChars`: Specifies the screen buffer height, if no GUI window is created this is ignored (NULL).
* `dwFillAttribute`: Specifies the inital text and background colors, if no GUI window is created this is ignored (NULL).
* `dwFlags`: Specifies startup window parameters (Bitmask), used when a process makes a window.
* `wShowWindow`: Specifies the STARTF_USESHOWWINDOW parameter, can show any value from nCmdShow parameter of the [ShowWindow](https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-showwindow) function, otherwise it is ignored. (NULL)
* `cbReserved2`: Reserved must be zero (NULL)
* `lpReserved2`: Reserved must be NULL (NULL)
* `hStdInput`: Specifies handle to use for stdin 
* `hStdOutput`: Specifies handle to use for stdout
* `hStdError`: Specifies handle to use for stderr


The `_PROCESS_INFORMATION` structure definition is shown below:
```
...
```
* `hProcess`: Handel to newly created process.
* `hThread`: Handel to primary thread of the newly created process.
* `dwProcessId`: Process ID that uniquely identifies the process.
* `dwThreadId`: Thread ID, used to uniquely identify a thread. Once a thread exits the ID may be reused.  
